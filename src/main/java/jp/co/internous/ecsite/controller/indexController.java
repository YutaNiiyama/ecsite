...
public class indexController {
	...
	@Autowired
	private TblPurchaseMapper purchaseMappser;
	...
	public int purchaseApi(@RequestBody CartForm f) {
		...
		f.getCartList().forEach((c) -> {
			...
			purchaseMappser.insert(f.getUserId(), c.getId(), c.getGoodsName(), c.getCount(), total);
		});
	...
	public String historyApi(@RequestBody HistoryForm f) {
		...
		List<HistoryDto> history = purchaseMappser.findHistory(userId);
